...
per-client buttons are **Schedule / Workouts / Membership / Daily Check-in** (plus
Meal plan, Supplements, Progress, Reports, Payments, Notes). Trainer top nav adds
**Analytics** and **Announce**.

- **Session ratings were removed** (client + trainer rating screens). The DB columns
  stay, unused. Rating now lives on **Daily Check-in**: `diet_photos.trainer_diet_rating`
...
- **Announcements** (`announcements`) — trainer posts a message (**Announce** tab);
...
- **Diet photos** (`diet_photos`) — one entry per day, JSON array of up to 10
  ...
  whole day**, stored with its timestamp. (No 24h auto-delete job yet.)
...
Create a trainer and a couple of clients from the **Sign up** page (choose the role
in the dropdown). Trainers land on the client list; clients land on their profile.
...
# Create two clients and one trainer
A=$(curl -s -X POST $API/auth/signup -H 'Content-Type: application/json' \
  -d '{"name":"Client A","phone_number":"1111","password":"secret1","role":"client"}')
B=$(curl -s -X POST $API/auth/signup -H 'Content-Type: application/json' \
  -d '{"name":"Client B","phone_number":"2222","password":"secret2","role":"client"}')
T=$(curl -s -X POST $API/auth/signup -H 'Content-Type: application/json' \
  -d '{"name":"Tina","phone_number":"9999","password":"trainer1","role":"trainer"}')
...
```
Runs **100 assertions**: phone/password validation, password reset + change,
self-service profile (photo + feeling note), the full access matrix, client CRUD,
sessions + **the removed rating endpoints now 404**, **diet-discipline rating** on
Daily Check-in, the meal-plan word limit, notes, progress / measurements, reports,
**packages with automatic `sessions_used` bookkeeping**, payments, **analytics**
(attendance / monthly revenue / low-session count), **announcements** (latest wins),
and **client-to-client isolation** on every table.

## How to test (in the browser)

The server seeds **trainer "Salman"** (`9000000001` / `trainer1`) and a demo client
**"Test Client"** (`8000000001` / `client1`) with a **1-session membership**, so the
last-session banner is visible immediately. Create a second client to check isolation.

- **Tab renames** — trainer client page buttons read **Schedule / Workouts /
  Membership / Daily Check-in**; client nav has **Workouts** and **Daily Check-in**.
...
- **Diet-discipline rating** — client → **Daily Check-in** → upload a photo. Trainer →
  client → **Daily Check-in** → set the star rating + comment → client sees both under
  "Trainer review".
...
- **Announcements** — Salman → **Announce** → post a message → client dashboard shows
...
|--------|------|------|-------|
| POST | `/auth/signup` · `/auth/login` | none | phone = 10 digits, password ≥ 6 (`422` otherwise) |
...
| PATCH | `/users/me` | self | `{profile_photo_url?, feeling_note?}` — client self-service |
