...
export default function ClientSessions() {
  ...
  function load() {
    api.listSessions(id).then((rows) => {
      ...
      setSessions(sorted);
      const d = Object.fromEntries(sorted.map((s) => [s.id, s.workout_details || ""]));
      setDrafts(d);
      savedDrafts.current = { ...d };
  ...
  const removeSession = (sid) => {
    if (window.confirm("Delete this session?")) run(() => api.deleteSession(id, sid));
  };

  const dirty = Object.keys(drafts).some((sid) => drafts[sid] !== savedDrafts.current[sid]);

  async function saveDetails() {
    setError("");
    try {
      for (const sid of Object.keys(drafts)) {
        if (drafts[sid] !== savedDrafts.current[sid]) {
          await api.updateSession(id, sid, { workout_details: drafts[sid] || null });
        }
      }
      load();
    } catch (e) {
    ...
    <div className="card">
      ...
      {sessions === null ? (
        ...
        <>
            ...
            <tbody>
              {sessions.map((s) => (
                <tr key={s.id}>
                  <td>
                    {s.date}
                    <div className="muted" style={{ fontSize: "0.75rem" }}>{DAYS[isoWeekday(s.date)]}</div>
                  </td>
                  <td>
          ...
          <SaveBar dirty={dirty} onSave={saveDetails} label="Save workout details" />
        </>
      )}
    </div>
  );
